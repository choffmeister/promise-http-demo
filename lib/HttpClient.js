  ...
  return new Bluebird(function (resolve, reject) {
    ...
    var req = http.request(reqOptions, function (res) {
      if (res.statusCode >= 200 && res.statusCode < 300) {
        var body = new Buffer(options.maxBodySize, 'raw');
        var bodyLength = 0;
        res.on('data', function (chunk) {
          if (bodyLength + chunk.length <= options.maxBodySize) {
        ...
        res.on('end', function () {
          resolve(self.transformResponseBody(body.slice(0, bodyLength)));
        });
...
module.exports = {
  Raw: HttpClient,
  Json: function (baseUrl) {
    return new HttpClient(baseUrl,
      function (raw) {
        return new Buffer(JSON.stringify(), 'utf-8');
      },
      function (raw) {
